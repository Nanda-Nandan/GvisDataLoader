...
from file_io1 import PathManager
...
def load_ytvis_json1(json_file, image_root, dataset_name=None, extra_annotation_keys=None):
    from ytvis_api.ytvos import YTVOS
...
"""
    ...
    vids = ytvis_api.loadVids(vid_ids)
    img_ids = []
    for idx,vid_info in enumerate(vids):
...
if __name__ == "__main__":
    ...
    meta =_get_ytvis_2021_instances_meta()
    json_file = r"D:\youtube-vis201\train\train\instances.json"
    image_root = r"D:\youtube-vis201\train\train\JPEGImages"
    ...
    for d in dicts:
        ...
        vid_name = d["file_names"][0].split('\\')[-1].split('/')[0]
        # if vid_name!="0bebb693b9":
        #     continue
        os.makedirs(os.path.join(dirname, vid_name), exist_ok=True)
        for idx, file_name in enumerate(d["file_names"]):
            annotations = d["annotations"][idx]
            for annotation in annotations:
                if annotation["id"] not in color_map.keys():
                    color_map[annotation["id"]] = random_color(rgb=True, maximum=1)
            img = np.array(Image.open(file_name))
            img1 = img.copy()
            visualizer = Visualizer(img1, metadata=meta)
            visualizer.color_map = color_map
